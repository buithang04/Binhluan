...
export default function AdminDashboardPage() {
  ...
  const load = useCallback(async (opts?: { silent?: boolean }) => {
    ...
    try {
      ...
      if (r.status === 401) {
        // Chỉ báo lỗi — SessionKeepAlive / middleware xử lý logout thật.
        // Tránh signOut ngay khi race reload (session chưa kịp hydrate).
  ...
  useEffect(() => {
    ...
    void load();
    // Chỉ load khi đăng nhập xong — không phụ thuộc object session (tránh refetch khi update)
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="page-desc">Theo dõi bình luận Maps đã đăng theo gói (completed / hạn mức gói).</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="panel overflow-x-auto">
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.id}>
                  ...
                  <td className="font-mono text-xs text-[var(--muted)]">{r.userEmail}</td>
                  <td>
          ...
          {rows.length === 0 && !error && (
            <p className="px-4 py-10 text-center text-sm text-[var(--muted)]">Chưa có dự án</p>
          )}
        </div>
      )}
    </div>
